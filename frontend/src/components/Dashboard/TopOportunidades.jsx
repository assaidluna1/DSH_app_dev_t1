  ...
  return (
    <div className="bg-surface-900 border border-surface-800 rounded-2xl p-6 shadow-xl">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-base font-bold text-surface-100">Top 10 Oportunidades</h3>
          <p className="text-xs text-surface-400 mt-0.5">Operaciones de mayor impacto financiero en el pipeline</p>
        </div>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs text-surface-300">
          <thead className="bg-surface-950/60 text-[11px] uppercase tracking-wider text-surface-400 border-b border-surface-800">
            <tr>
          ...
          <tbody className="divide-y divide-surface-800/60">
            {data.map((opp) => (
              ...
              >
                <td className="py-3 px-3 font-medium text-surface-100 group-hover:text-brand-400 transition-colors">
                  {opp.nombre}
                </td>
                ...
                <td className="py-3 px-3 text-right font-bold text-surface-100">
                  {formatCurrency(opp.valor_estimado_usd)}
                </td>
                <td className="py-3 px-3 text-center">
                  <span className="font-semibold text-brand-400">{Math.round(opp.probabilidad)}%</span>
                </td>
                <td className="py-3 px-3 text-surface-400 font-mono text-[11px]">
                  {opp.fecha_cierre_estimada || 'Sin fecha'}
                </td>
                <td className="py-3 px-3 text-surface-300">
                  {opp.propietario?.nombre || '—'}
                </td>
                <td className="py-3 px-2 text-center text-surface-500 group-hover:text-surface-200">
